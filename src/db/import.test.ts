...
describe('importCardsToNewDeck', () => {
  ...
  it('パース済みカードを一括インポートできる', async () => {
    const parsed: ParsedCard[] = [
      { front: 'こんにちは', back: '안녕하세요', tags: ['挨拶'], difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: 'ありがとう', back: '감사합니다', tags: ['挨拶'], difficulty: 2, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
    ]
    ...
    expect(cards.find((c) => c.front === 'こんにちは')?.tags).toEqual(['挨拶'])
    expect(cards.find((c) => c.front === 'ありがとう')?.difficulty).toBe(2)
  })

  it('重複する front は最初の1件のみインポートし残りをスキップする', async () => {
    const parsed: ParsedCard[] = [
      { front: 'トイレ', back: '화장실', tags: ['場所'], difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: 'こんにちは', back: '안녕하세요', tags: ['挨拶'], difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: 'トイレ', back: '화장실', tags: ['単語'], difficulty: 2, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
    ]
    const result = await importCardsToNewDeck('テスト', parsed)
  ...
  it('学習データ (correctCount, incorrectCount, lastStudiedAt) も保持される', async () => {
    ...
    const parsed: ParsedCard[] = [
      { front: 'apple', back: 'りんご', tags: ['果物'], difficulty: 3, correctCount: 5, incorrectCount: 2, lastStudiedAt: ts },
    ]
    const result = await importCardsToNewDeck('テスト', parsed)
  ...
  it('全行が同じ front でも最初の1行だけインポートされる', async () => {
    const parsed: ParsedCard[] = [
      { front: 'a', back: '1', tags: [], difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: 'a', back: '2', tags: [], difficulty: 2, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: 'a', back: '3', tags: [], difficulty: 3, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
    ]
    const result = await importCardsToNewDeck('テスト', parsed)
    expect(result.imported).toBe(1)
...
describe('mergeImportCards', () => {
  ...
  it('既存カードがある場合は上書き更新される', async () => {
    ...
    const parsed: ParsedCard[] = [
      { front: 'こんにちは', back: '안녕하세요', tags: ['挨拶', '基本'], difficulty: 3, correctCount: 10, incorrectCount: 2, lastStudiedAt: null },
    ]
    const result = await mergeImportCards('テスト', parsed)
    ...
    expect(card!.tags).toEqual(['挨拶', '基本'])
    expect(card!.difficulty).toBe(3)
  ...
  it('新規と既存が混在する場合、それぞれ追加と更新が行われる', async () => {
    ...
    const parsed: ParsedCard[] = [
      { front: 'こんにちは', back: '안녕하세요', tags: ['挨拶'], difficulty: 2, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: 'ありがとう', back: '감사합니다', tags: ['挨拶'], difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
      { front: '水', back: '물', tags: ['単語'], difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null },
    ]
    const result = await mergeImportCards('テスト', parsed)
    expect(result.added).toBe(2)
    expect(result.updated).toBe(1)
  ...
  it('学習データ付きの新規カードも correctCount/incorrectCount が反映される', async () => {
    ...
    const parsed: ParsedCard[] = [
      { front: 'dog', back: '犬', tags: [], difficulty: 1, correctCount: 8, incorrectCount: 3, lastStudiedAt: ts },
    ]
    const result = await mergeImportCards('テスト', parsed)
...
describe('TSV パース → DB インポート統合テスト', () => {
  ...
  it('サンプルデッキ形式の TSV をパース＆インポートできる', async () => {
    ...
    const greetings = cards.filter((c) => c.tags.includes('挨拶'))
    expect(greetings).toHaveLength(2)

    const place = cards.find((c) => c.tags.includes('場所'))!
  ...
  it('重複 front を含むサンプル TSV でも最初の1件だけインポートされる', async () => {
    ...
    expect(card!.tags).toEqual(['場所'])
  })

  it('旧形式 TSV (tags/difficulty なし) からのインポートもデフォルト値が設定される', async () => {
    const tsv = [
  ...
  it('ヘッダなし TSV からのインポートでもカードが作成される', async () => {
    ...
    const cards = await getCardsForDeck('テスト')
    expect(cards).toHaveLength(2)
    expect(cards.every((c) => c.tags.length === 0)).toBe(true)
