...
describe('generateTsv', () => {
  it('カードの配列から TSV 文字列を生成する', () => {
    const cards: Card[] = [
      { deckName: '英単語', front: 'apple', back: 'りんご', tag: '果物', difficulty: 2, correctCount: 5, incorrectCount: 2, lastStudiedAt: new Date('2026-02-04T10:30:00Z').getTime(), createdAt: 0, updatedAt: 0 },
      { deckName: '英単語', front: 'book', back: '本', tag: '', difficulty: 1, correctCount: 3, incorrectCount: 0, lastStudiedAt: new Date('2026-02-03T15:00:00Z').getTime(), createdAt: 0, updatedAt: 0 },
    ]
    const tsv = generateTsv(cards)
    const lines = tsv.split('\n')
    expect(lines[0]).toBe('front\tback\ttag\tdifficulty\tcorrectCount\tincorrectCount\tlastStudiedAt')
  ...
  it('未学習カードの lastStudiedAt は空文字', () => {
    const cards: Card[] = [
      { deckName: '英単語', front: 'dog', back: '犬', tag: '', difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null, createdAt: 0, updatedAt: 0 },
    ]
    const tsv = generateTsv(cards)
    const lines = tsv.split('\n')
  ...
  it('空配列ならヘッダ行のみ', () => {
    ...
    expect(tsv).toBe('front\tback\ttag\tdifficulty\tcorrectCount\tincorrectCount\tlastStudiedAt')
  })
})
...
describe('parseTsv', () => {
  it('ヘッダ付き TSV をパースして front/back を取得する', () => {
    ...
    expect(result[0]).toEqual({ front: 'apple', back: 'りんご', tag: '', difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null })
    expect(result[1]).toEqual({ front: 'book', back: '本', tag: '', difficulty: 1, correctCount: 0, incorrectCount: 0, lastStudiedAt: null })
  })
  ...
  it('tag と difficulty がない旧形式 TSV もパースできる', () => {
  ...
  it('difficulty が範囲外の場合は 1 にフォールバックする', () => {
    const tsv = 'front\tback\ttag\tdifficulty\napple\tりんご\t\t0\nbook\t本\t\t6'
    const result = parseTsv(tsv)
    expect(result[0].difficulty).toBe(1)
